...
export default async function FormUpdateLocation({store}: {store: string | string[] | undefined}) {
    ...
    return(
        <form action={createLocation} className="bg-orange-400 py-2 px-4 flex flex-col gap-6 w-full rounded-lg">
            ...
            <Input defaultValue={foundLocation?.locationName} label="Nombre" placeholder="oxxo Jurica" name="locationName"/>
            <Input defaultValue={foundLocation?.locationAddress} label="Dirección" placeholder="Av Tlacote S/N" name="locationAddress"/>
